...
#define MAGIC_PORT 34567 // Port from which UDP and TCP packets are being sent
 ...
 */
void udp_send(std::string ip_addr, int interface_index);
 ...
 *
 * @param ip_addr
 * @param interface_index - index of interface to scan
 *
 * @return Void.
 *
 */
 ...
 * @return Void.
 *
 * @bug If sendto sets errno to EAGAIN or ENOBUFS, there are no available resources. Keeps trying until successful.
 */
void tcp_send(std::string ip_addr, int interface_index);
 ...
 *
 * @param buffer - data to perform checksum upon
 * @param size - volume of data
 *
 * @return Void.
 *
 */
