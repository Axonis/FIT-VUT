...
**/
	...
	tBTNodePtr ins_tmp = NULL;

	if(tmp == NULL) { // ak je prazdny
		...
		*RootPtr = tmp;
	...
	else { // vkladame do existujuceho stromu
		while(tmp != NULL){ // hladame miesto

			ins_tmp = tmp; // koren sa nastavi ako jeho otec

			if(tmp->Cont > Content) // ak je vkladany mensi ide vlavo
				tmp = tmp->LPtr;
			else if(tmp->Cont < Content) // ak je vkladany vacsi ide vpravo
				tmp = tmp->RPtr;
			else
				return;
		...
		tmp = malloc(sizeof(struct tBTNode));
		if(tmp == NULL) // ak sa nepodarilo alokovat
			return;
		else {
			tmp->LPtr = NULL; // nema lavy podstrom
			tmp->RPtr = NULL; // nema pravy
			tmp->Cont = Content; // vlozim obsah

			if(ins_tmp->Cont < Content) // pripajam pravy
				ins_tmp->RPtr = tmp;
			else // pripajam lavy
				ins_tmp->LPtr = tmp;
		}
...
**/
	...
	while(ptr != NULL) { // pokial mam co prechadzat
		BTWorkOut(ptr); // spracujem uzol
		SPushP(Stack, ptr); // pushnem ukazatel na uzol
		ptr = ptr->LPtr; // pokracujem dolava
	}
}
...
**/
	...
	Leftmost_Preorder(RootPtr, &stack); // spracujeme vsetky najlavejsie uzly

	while(!SEmptyP(&stack)) // kym je nieco v zasobniku
		Leftmost_Preorder(STopPopP(&stack)->RPtr, &stack); // spracujeme najlavejsie uzly praveho podstromu spracovavaneho uzlu
...
**/
	...
	tBTNodePtr tmp; // pomocna premenna

	Leftmost_Inorder(RootPtr, &stack); // ulozime vsetky najlavejsie uzly

	while(!SEmptyP(&stack))	{ // kym je nieco v zasobniku
		tmp = STopPopP (&stack) ;
		BTWorkOut(tmp); // spracujem uzol
		Leftmost_Inorder(tmp->RPtr, &stack); // ulozime najlavejsie uzly praveho podstromu spracovavaneho uzlu
	}
}
...
**/
	...
	while(ptr != NULL) {// pokial mam co prechadzat
		SPushP(StackP, ptr); // pushnem ukazatel na uzol
		SPushB(StackB, true); // nastavime priznak ze sa navstivil este len prvykrat
		ptr = ptr->LPtr; // pokracujem dolava
	}
...
**/
	...
	while(!SEmptyP(&StackP)){
		tmp = STopPopP(&StackP); // pozrie sa na prvok z vrchola stacku
		SPushP(&StackP, tmp); // vrati ho naspat

		if(STopPopB(&StackB)){ // ak v tom uzli este nebol
			SPushB(&StackB,false); // nastavi ze uz bol
			Leftmost_Postorder(tmp->RPtr, &StackP, &StackB); // ide vpravo
		}
		else { // ak uz v uzli bol
			tmp = STopPopP(&StackP);
			BTWorkOut(tmp); // spracuje uzol
		}
...
**/
	...
	while(*RootPtr != NULL || (!SEmptyP(&stack))){ // ak strom a zasobnik nieje prazdny

		if (*RootPtr == NULL && !(SEmptyP(&stack))) // ak je strom prazdny odoberieme ukazatel
			*RootPtr = STopPopP(&stack);
		else{ // ak nieje prazdny
			if((*RootPtr)->RPtr != NULL) // ak ma pravy podstrom
				SPushP(&stack,(*RootPtr)->RPtr); // pushni na neskor

			tmp = *RootPtr; // ulozenie korena do pomocnej
