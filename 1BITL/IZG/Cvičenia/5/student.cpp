                     ...
                     const S_Material &material)
{
  ...
  //pozicia kamery - suradnice aa vektoru
  S_Coords S(0-aa.x, 0-aa.y, -CAMERA_DIST-aa.z );

  //podla kamery a jej normaly urcim ci vidim zadnu stenu
  if (((S.x * nn.x) + (S.y * nn.y) + (S.z * nn.z)) < 0)
      return;
...
void DrawScene()
{
  ...
  //zoberie maticu po translacii sceny
